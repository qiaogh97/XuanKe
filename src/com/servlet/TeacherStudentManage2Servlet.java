...
public class TeacherStudentManage2Servlet extends HttpServlet {
	@Override
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
	        ...
	        for(int i=0;i<listsint.size();i++){
	        	ArrayList<Stu_lab> selectStu_lab2 = stu_LabDao.selectStu_lab2(listsint.get(i), 0);//根据该编号依次查询数据
	        	for(int j =0 ;j<selectStu_lab2.size();j++){
	        		stu_labList.add(selectStu_lab2.get(j));
	        	}
	        }
			req.setAttribute("stu_labList", stu_labList);
			RequestDispatcher rd = req.getRequestDispatcher("tea2_stumanage2.jsp");  
			try {  
			    rd.forward(req, resp);  
			         return;  
			}catch(Exception e){
